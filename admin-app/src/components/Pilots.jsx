...
export default function Pilots() {
  ...
  function startEdit(p) {
    setEditingId(p._id);
    setEditForm({ name: p.name, phone: p.phone || '', password: '', active: p.active });
  }

  async function saveEdit(id) {
    // Only send password when a new one was typed.
    const body = { name: editForm.name, phone: editForm.phone, active: editForm.active };
  ...
  return (
    <div>
      <h1>Pilots</h1>
      <p className="muted">Pilots log in to the pilot app with these credentials.</p>
